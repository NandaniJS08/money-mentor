...
export default function ProfilePage() {
  ...
  const [formData, setFormData] = useState({
    ...
    age: "24",
    annualIncome: "12,0,000",
    monthlyExpense: "45,000",
    goal: "Buying a Flat",
    experience: "Mid"
  });
    ...
    >
      ...
      <motion.section variants={itemVariants} whileHover={{ y: -4 }} className={`${s.card} rounded-[2.5rem] border p-8 space-y-8`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            <label className={s.label}>Annual Income (₹)</label>
            <input type="text" value={formData.annualIncome} onChange={(e) => setFormData({...formData, annualIncome: e.target.value})} className={`w-full p-4 rounded-2xl outline-none border font-mono ${s.input}`} />
          </div>
          <div>
